...
class Bomber(object):
    ...
    def set_agent_id(self, agent_id):
        ...
        if self._game_type == GameType.FFA:
            self.teammate = Item.AgentDummy
            self.enemies = [
                getattr(Item, 'Agent%d' % id_)
                for id_ in range(4)
                if id_ != agent_id
            ]
        else:
            teammate_id = (agent_id + 2) % 4
            self.teammate = getattr(Item, 'Agent%d' % teammate_id)
            self.enemies = [
                getattr(Item, 'Agent%d' % id_)
                for id_ in range(4)
                if id_ != agent_id and id_ != teammate_id
            ]
            self.enemies.append(Item.AgentDummy)

    def maybe_lay_bomb(self):
        if self.ammo > 0:
            self.ammo -= 1
            return Bomb(self, self.position, DEFAULT_BOMB_LIFE,
                        self.blast_strength)
        return None
    ...
    def get_next_position(self, direction):
        ...
        return get_next_position(self.position, action)
    ...
    def reset(self, ammo=1, is_alive=True, blast_strength=None, can_kick=False):
        ...
        self.blast_strength = blast_strength or DEFAULT_BLAST_STRENGTH
...
class Bomb(object):
    ...
    def move(self):
        if self.is_moving():
            self.position = get_next_position(self.position,
